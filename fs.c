...
i32 fsRead(i32 fd, i32 numb, void* buf) {
  ...
  i32 fdRead = numb;
  i32 fdInum = bfsFdToInum(fd);
  i32 fdSize = bfsGetSize(fdInum);
  i32 fdPos = bfsTell(fd);
  i32 fbnStart = fdPos / BYTESPERBLOCK;
  i32 fbnEnd = (fdPos + numb) / BYTESPERBLOCK;
  if (fdPos + numb > fdSize) {
    fdRead = fdSize - fdPos;
    fbnEnd = (fdPos + fdRead) / BYTESPERBLOCK;
  }
  i8 blocks = fbnEnd - fbnStart + 1;
  i8 tempBuffer[blocks * BYTESPERBLOCK];
  for(i32 i = fbnStart; i <= fbnEnd; i++) {
    bfsRead(fdInum, i, tempBuffer + (i - fbnStart) * BYTESPERBLOCK);
  }
  memcpy(buf, tempBuffer + fdPos % BYTESPERBLOCK, fdRead);
  fsSeek(fd, fdRead, SEEK_CUR);
  return fdRead;
}
...
i32 fsWrite(i32 fd, i32 numb, void* buf) {
  i32 fdInum = bfsFdToInum(fd);
  i32 fdSize = bfsGetSize(fdInum);
  i32 fbnSize = (fdSize - 1) / BYTESPERBLOCK;
  i32 fdPos = bfsTell(fd);
  i32 fbnStart = fdPos / BYTESPERBLOCK;
  i32 fbnEnd = (fdPos + numb) / BYTESPERBLOCK;
  bool expandedWrite = false;
  if (fbnEnd > fbnSize) {
    expandedWrite = true;
    bfsExtend(fdInum, fbnEnd);
  }
  i8 blocks = fbnEnd - fbnStart + 1;
  i8 tempBuffer[blocks * BYTESPERBLOCK];
  for (i32 i = fbnStart; i <= fbnEnd; i++) {
  ...
  memcpy(tempBuffer + fdPos % BYTESPERBLOCK, buf, numb);
  for (i32 i = fbnStart; i <= fbnEnd; i++) {
    i32 writePos = bfsFbnToDbn(fdInum, i);
    bioWrite(writePos, tempBuffer + (i - fbnStart) * BYTESPERBLOCK);
  }
  if (expandedWrite == true) {
    bfsSetSize(fdInum, fdPos + numb);
  }
